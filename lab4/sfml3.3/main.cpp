            ...
            sf::ConvexShape& pupilFirst, sf::ConvexShape& eyeSecond, sf::ConvexShape& pupilSecond) 
{
    ...
    if(std::hypot(deltaFirstPupil.x, deltaFirstPupil.y) <= std::hypot(pupilFirstOffset.x, pupilFirstOffset.y))
    {
        pupilFirst.setPosition(mousePosition);
    }
    if(std::hypot(deltaSecondPupil.x, deltaSecondPupil.y) <= std::hypot(pupilSecondOffset.x, pupilSecondOffset.y))
    {
        pupilSecond.setPosition(mousePosition);
    }
}
